...
const OrganizationsSection: React.FC<OrganizationsSectionProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-white border border-surface-100 rounded-2xl p-6 shadow-[0_16px_34px_rgba(15,23,42,0.08)]">
        <h3 className="text-lg font-black mb-4">CADASTRAR ORGANIZAÇÃO</h3>
        <form onSubmit={handleCreateOrganization} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-surface-500 mb-2 block">Nome da organização</label>
            <input
          ...
          <label className="flex items-center gap-2 text-sm text-surface-600 font-semibold">
            <input
              type="checkbox"
          ...
          <button type="submit" className="px-4 py-3 rounded-xl bg-brand-600 hover:bg-brand-500 font-bold">
            Salvar organização
          </button>
        </form>
      </div>
      ...
      <div className="bg-white border border-surface-100 rounded-2xl p-6 shadow-[0_16px_34px_rgba(15,23,42,0.08)]">
        ...
        <div className="space-y-3">
          {organizations.map((organization) => {
            ...
            return (
              <div key={organization.id} className="p-3 rounded-xl bg-surface-50 border border-surface-100 space-y-3 shadow-[0_10px_22px_rgba(15,23,42,0.06)]">
                {isEditing ? (
                  <>
                    <input
                      value={editingOrganizationName}
                      onChange={(event) => setEditingOrganizationName(event.target.value)}
                      className="w-full p-2 bg-white border border-surface-200 rounded-lg text-surface-800 font-semibold"
                    />
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="px-3 py-2 rounded-lg bg-surface-100 hover:bg-surface-200 text-surface-700 text-xs font-bold"
                      >
                        Cancelar
                      </button>
                    </div>
                  ...
                  <>
                    ...
                    <p className="text-xs text-surface-500">ID: {organization.id}</p>
                    <div className="flex gap-2 pt-1 flex-wrap">
                      <button
                        type="button"
                        onClick={() => handleStartEditOrganization(organization)}
                        className="px-3 py-2 rounded-lg bg-brand-600 hover:bg-brand-500 text-xs font-bold"
                      >
                        Editar
                      </button>
                      <button
          ...
          {organizations.length === 0 && (
            <div className="col-span-full flex flex-col items-center py-12 text-center">
              ...
              <p className="text-sm font-bold text-surface-500">Nenhuma organização cadastrada</p>
              <p className="text-xs text-surface-400 mt-1">Crie sua primeira organização no formulário ao lado.</p>
            </div>
          )}
        </div>
      </div>
