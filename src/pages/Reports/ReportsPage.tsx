    ...
    <section className="space-y-4">
      <div className="rounded-2xl border border-surface-100 bg-white p-4 shadow-sm">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
          <input type="date" value={filters.periodStart || ''} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, periodStart: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm" />
          <input type="date" value={filters.periodEnd || ''} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, periodEnd: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm" />
          <select value={filters.processStatus || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, processStatus: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
            <option value="all">Todos os status</option>
            {result?.options.statuses.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
          </select>
          <select value={filters.processType || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, processType: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
            <option value="all">Todos os tipos</option>
            {result?.options.types.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
          </select>
          <select value={filters.responsibleUserId || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, responsibleUserId: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
            <option value="all">Todos os responsáveis</option>
            {result?.options.responsibles.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
          </select>
          <select value={filters.actorUserId || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, actorUserId: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
          ...
          {organizationFilterEnabled ? (
            <select value={filters.organizationId || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, organizationId: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
              <option value="all">Todas as organizações</option>
              {result?.options.organizations.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}
            </select>
          ) : null}
          <select value={filters.eventType || 'all'} onChange={(e) => { setPage(1); setFilters((p) => ({ ...p, eventType: e.target.value })); }} className="rounded-lg border border-surface-200 p-2 text-sm">
        ...
        <div className="mt-3 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="relative w-full md:max-w-xl">
            ...
            <input
              ...
              className="w-full rounded-lg border border-surface-200 py-2 pl-9 pr-3 text-sm"
            />
          </div>
          <div className="flex gap-2">
            <select
              ...
              className="rounded-lg border border-surface-200 px-2 py-2 text-xs font-semibold"
            >
              <option value="desc">Mais recente</option>
              <option value="asc">Mais antigo</option>
            </select>
            <button onClick={() => void exportCsv()} className="inline-flex items-center gap-2 rounded-lg bg-brand-600 px-3 py-2 text-xs font-bold text-white"><Download className="h-4 w-4" /> CSV</button>
            <button onClick={() => void exportPdf()} className="inline-flex items-center gap-2 rounded-lg bg-surface-800 px-3 py-2 text-xs font-bold text-white"><FileText className="h-4 w-4" /> PDF</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Total filtrado</p><p className="text-2xl font-black text-surface-900">{result?.summary.total || 0}</p></div>
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Status (top)</p><p className="text-sm font-bold text-surface-900">{result?.summary.byStatus.sort((a,b)=>b.total-a.total)[0]?.key || '-'}</p><p className="text-xs text-surface-500">{result?.summary.byStatus.sort((a,b)=>b.total-a.total)[0]?.total || 0}</p></div>
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Evento (top)</p><p className="text-sm font-bold text-surface-900">{result?.summary.byEventType.sort((a,b)=>b.total-a.total)[0]?.key || '-'}</p><p className="text-xs text-surface-500">{result?.summary.byEventType.sort((a,b)=>b.total-a.total)[0]?.total || 0}</p></div>
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Usuário ator (top)</p><p className="text-sm font-bold text-surface-900">{result?.summary.byActor.sort((a,b)=>b.total-a.total)[0]?.key || '-'}</p><p className="text-xs text-surface-500">{result?.summary.byActor.sort((a,b)=>b.total-a.total)[0]?.total || 0}</p></div>
      </div>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Consolidado por organização (top)</p><p className="text-sm font-bold text-surface-900">{result?.summary.byOrganization.sort((a,b)=>b.total-a.total)[0]?.key || '-'}</p><p className="text-xs text-surface-500">{result?.summary.byOrganization.sort((a,b)=>b.total-a.total)[0]?.total || 0}</p></div>
        <div className="rounded-xl border border-surface-100 bg-white p-4"><p className="text-xs text-surface-500">Consolidado por usuário (top)</p><p className="text-sm font-bold text-surface-900">{result?.summary.byUser.sort((a,b)=>b.total-a.total)[0]?.key || '-'}</p><p className="text-xs text-surface-500">{result?.summary.byUser.sort((a,b)=>b.total-a.total)[0]?.total || 0}</p></div>
      </div>
          ...
          <>
                      ...
                      <React.Fragment key={row.process.id}>
                        <tr className="border-t border-surface-100">
                          ...
                          <td className="px-3 py-2">
                            <button onClick={() => setExpandedProcessId((current) => current === row.process.id ? null : row.process.id)} className="inline-flex items-start gap-1 text-left text-brand-600 hover:underline">
                        ...
                        {isExpanded ? (
                          <tr className="border-t border-surface-100 bg-surface-50">
                            <td className="px-3 py-2" colSpan={9}>
                              <p className="mb-2 text-xs font-bold uppercase text-surface-500">Timeline detalhada</p>
                              ...
                              <p className="mt-3 mb-1 text-xs font-bold uppercase text-surface-500">Financeiro</p>
                              <ul className="space-y-1 text-xs text-surface-700">
                              ...
                              <p className="mt-3 mb-1 text-xs font-bold uppercase text-surface-500">Pagamentos</p>
                              <ul className="space-y-1 text-xs text-surface-700">
                              ...
                              <p className="mt-3 mb-1 text-xs font-bold uppercase text-surface-500">Documentos/Anexos</p>
                              <ul className="space-y-1 text-xs text-surface-700">
            ...
            <div className="block md:hidden space-y-2 p-3">
              {result?.rows.map((row) => {
                const isExpanded = expandedProcessId === row.process.id;
                return (
                  <div key={row.process.id} className="border border-surface-100 rounded-xl p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-bold text-surface-900 text-sm">{row.process.titulo}</p>
                        <span className="text-[10px] font-black bg-brand-100 text-brand-700 px-2 py-0.5 rounded inline-block mt-1">{row.process.protocolo || '-'}</span>
                      </div>
                      <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-surface-100 text-surface-600 whitespace-nowrap">{row.process.process_status || row.process.status || '-'}</span>
                    </div>
                    <div className="space-y-1 text-xs text-surface-600">
                      <p><span className="font-semibold text-surface-400">Cliente:</span> {row.process.cliente_nome || '-'}</p>
                      <p><span className="font-semibold text-surface-400">Tipo:</span> {row.process.unidade_atendimento || '-'}</p>
                      <p><span className="font-semibold text-surface-400">Responsável:</span> {row.responsibleName}</p>
                      <p><span className="font-semibold text-surface-400">Usuário ator:</span> {row.actorName}</p>
                      <p><span className="font-semibold text-surface-400">Org.:</span> {row.organizationName}</p>
                      {row.latestEvent?.mensagem && (
                        <p className="pt-2 mt-2 border-t border-surface-100"><span className="font-semibold text-surface-400">Último evento:</span> {row.latestEvent.mensagem}</p>
                      )}
                    </div>
                    <button onClick={() => setExpandedProcessId((current) => current === row.process.id ? null : row.process.id)} className="mt-3 flex items-center gap-1 text-xs font-bold text-brand-600">
                    ...
                    {isExpanded && (
                      <div className="mt-3 pt-3 border-t border-surface-100 space-y-3">
                        <div>
                          <p className="text-xs font-bold uppercase text-surface-500 mb-1">Timeline detalhada</p>
                          <ul className="space-y-1 text-xs text-surface-700">
                            {row.events.map((event) => (
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase text-surface-500 mb-1">Financeiro</p>
                          <ul className="space-y-1 text-xs text-surface-700">
                            {row.financialHighlights.length ? row.financialHighlights.map((event, index) => (
                              <li key={`${event.type}-${index}`}>{event.createdAt ? new Date(event.createdAt).toLocaleString('pt-BR') : '-'} · {event.type} · {event.message}</li>
                            )) : <li>Nenhum evento financeiro relevante.</li>}
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase text-surface-500 mb-1">Pagamentos</p>
                          <ul className="space-y-1 text-xs text-surface-700">
                            {row.payments.length ? row.payments.map((payment) => (
                              <li key={payment.id}>{payment.createdAt ? new Date(payment.createdAt).toLocaleString('pt-BR') : '-'} · {payment.paymentStatus || 'sem status'}{payment.amount != null ? ` · ${formatEuro(Number(payment.amount))}` : ''}</li>
                            )) : <li>Nenhum pagamento vinculado.</li>}
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase text-surface-500 mb-1">Documentos/Anexos</p>
                          <ul className="space-y-1 text-xs text-surface-700">
                            {row.attachments.length ? row.attachments.map((attachment) => (
                              <li key={attachment.id}>{attachment.createdAt ? new Date(attachment.createdAt).toLocaleString('pt-BR') : '-'} · {attachment.name}</li>
                            )) : <li>Nenhum anexo registrado.</li>}
      ...
      <div className="flex items-center justify-between">
        <p className="text-xs font-semibold text-surface-500">Página {page} de {totalPages}</p>
        <div className="flex gap-2">
          <button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))} className="rounded border border-surface-200 px-3 py-1 text-xs font-bold disabled:opacity-50">Anterior</button>
          <button disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))} className="rounded border border-surface-200 px-3 py-1 text-xs font-bold disabled:opacity-50">Próxima</button>
        </div>
      </div>
    </section>
  );
